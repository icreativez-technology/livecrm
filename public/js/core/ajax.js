...
$(document).ready(function () {
	...
	$(document).on('click', '.addgoods', function (e) {		
		var goods = ['qty','units','kg_calc','ldm','value','description','volume','length','width','height']
	     $("#goodsTable").append('<tr id="row-'+rowIndex+j+'"></tr>');
		for(var i=0; i<goods.length; i++){
		  $("tr#row-"+rowIndex+j+"").append('<td><input type="text" class="form-control" name="goods['+rowIndex+']['+goods[i]+']" id="'+rowIndex+'"></td>');
		}
		$("tr#row-"+rowIndex+j+"").append('<td><button type="button" class="data-toggle-action-tooltip btn btn-outline-danger btn-circle btn-sm"  onclick="removeIndex(this)"><i class="sl-icon-trash"></i></button></td>');
		rowIndex++;
		j++;
	});
