...
<script>
$(function() {
    var calendar = $('#demo-week-select').mobiscroll().datepicker({
        controls: ['calendar'],
        select: 'preset-range',
        selectType: 'week',
        display: 'inline',
        ...
        onChange: function (event, inst) {
                const myArray = event.valueText.split(" ")
                var starDate = moment(myArray[0]).format('YYYY,MM,DD');
                var endDate = moment(myArray[2]).format('YYYY,MM,DD');
              $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                url: '<?php echo e(url('/')); ?>/tasks/<?php echo e($task->task_id); ?>/update-due-date',
                type: "POST",
    ...
    }).mobiscroll('getInst'); // Get the calendar instance
    var startDate = "<?php echo e($task->start_date); ?>";
    var endDate  = "<?php echo e($task->end_date); ?>";
    calendar.setVal([new Date(startDate), new Date(endDate)], true); // Set the selected week range
});

</script><?php /**PATH C:\xampp\htdocs\livesoft\livecrm\application\resources\views/pages/task/rightpanel.blade.php ENDPATH**/ ?>